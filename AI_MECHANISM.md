...
- **プロアクティブな挨拶**: ユーザーが5秒間話さない場合、AIから自動的に会話を開始
...
### 概要
接続確立後、ユーザーが5秒間話さない場合、AIが自動的に挨拶を開始する機能です。この機能により、ユーザーが何を言えばよいかわからない場合や、システムが正常に動作しているか不安な場合でも、スムーズに会話を開始できます。
...
async function sendProactiveGreeting() {
  ...
  if (!dc || userHasSpokenRef.current) return
...
#### 5. ユーザー音声検出（194-197行目、352-354行目）
...
### メリット
✅ **ユーザー体験の向上**: ユーザーが最初に何を言えばよいかわからない場合のガイダンス
✅ **システム確認**: 接続が正常に動作していることをユーザーに確認
✅ **自然な会話開始**: AIが積極的に会話を開始し、受動的な印象を軽減
✅ **柔軟性**: ユーザーが先に話す場合は通常通り動作
